...
@Entity
@NamedQueries(value = {
// @NamedQuery(name = "Material.getAll", query =
// "SELECT m FROM Material m ORDER BY m.name"),
		...
		// @NamedQuery(name = "Material.findByName", query =
		// "SELECT m FROM Material m JOIN m.materialCategory c WHERE LOWER(c.name) LIKE :name ORDER BY c.id"),
