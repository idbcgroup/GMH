 ...
 *
 */
public class EIATypeMaintenanceGridPanel extends VLayout implements GHAClosable,GHAHideable, EIATypeSelectionListener, MaintenancePlanSelectionListener{
	...
	public EIATypeMaintenanceGridPanel() {
		...
		Label title = new Label("<h3>Planes de Mantenimiento asociados al tipo de equipo</h3>");
		title.setHeight(30);
		title.setWidth100();
...
//						mainenanceProtocolForm.animateShow(AnimationEffect.FLY);
	...
	private void loadData() {
		EiaTypeMaintenancePlanModel.findByEiaType(this.eiaType, new GHAAsyncCallback<List<EiaTypeMaintenancePlan>>() {

			@Override
			public void onSuccess(List<EiaTypeMaintenancePlan> result) {
				ListGridRecord array[] = EiaTypeMaintenancePlanUtil.toMaintenancePlanGridRecords(result).toArray(new EiaTypeMaintenancePlanRecord[]{});
				grid.setData(array);
			}
		});
	}
