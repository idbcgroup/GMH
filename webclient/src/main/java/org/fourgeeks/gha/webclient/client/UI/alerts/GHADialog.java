...
public abstract class GHADialog extends Dialog implements ResizeHandler{
	...
	public enum DialogType {
		...
		SAY, 
		/**
		 * Type for a confirmation dialog. Gray, Modal, Untimed messages. Usually a question with 2 options: accept, cancel.
		 */
		CONFIRMATION,
		/**
		 * Type for an Ask yes/no dialog. Gray, Modal, Untimed messages. Usually a question with 3 options: yes, no, cancel.
		 */
		ASKYESNO,
		/**
		 * Type for a hard error dialog. Red, modal, untimed window that informs about a hard error. Has an accept and a copy button.
		 */
		ERROR_HARD,
		/**
		 * Type for a soft error dialog. Yellow, modal untimed message that informs about a soft error. Has an accept and a copy button.
		 */
		ERROR_SOFT,
		/**
		 * Type for a warning dialog. Blue, Non-modal, (optionally) timed message without buttons.
		 */
		WARNING,
		/**
		 * Type for an information dialog. Blue, Non-modal, (optionally) timed messages without buttons, for delivering info.
		 */
		INFORMATION,
		/**
		 * Type for a process failure dialog. Yellow, Non-modal, (optionally) timed, failure messages without buttons.
		 */
		FAILURE,
		/**
		 * Type for a process success dialog. Green, Non-modal, (optionally) timed, success messages without buttons.
		 */
		SUCCESS,
		/**
		 * Type for an operation-in-process dialog. Green, Non-modal, (optionally) timed messages without buttons, for giving feedback.
		 * can be minimized and can't be closed. the dialog closes when the operation finishes.
		 */
		PROCESSING,
		/**
		 * Type for an new_message dialog. Gray, Non-modal, (optionally) timed messages without buttons, for telling when a message arrives.
		 */
		NEW_MESSAGE;
	}
	 ...
	 * @param canMinimize 
	 */
	public GHADialog(DialogType type, boolean hasCloseButton, boolean canMinimize) {
		super();
		init(false, type);
		...
		//Label date = new Label(DateFormat.getDateInstance(DateFormat.TIMEZONE_FIELD).format(new Date()));
	 ...
	 * @param buttons
	 */
	public GHADialog(DialogType type, boolean hasCloseButton, boolean canMinimize, Button... buttons) {
		super();
		init(true, type);
	...
	public void onResize(ResizeEvent event) {
		if(isVisible()){
			if(hasButtons){
				...
				setTop(Window.getClientHeight()-(getHeight()+BORDER_SEPARATION));
			}else{
				setLeft(Window.getClientWidth()-(getWidth()+RIGHT_MARGIN));
				setTop(Window.getClientHeight()-(getHeight()+BORDER_SEPARATION));
			}
	...
	public void show() {
		...
		if(hasButtons){
			...
			getBody().setHeight(GHAUiHelper.DEFAULT_NOTIFICATION_BUTTONS_HEIGHT-(HEADER_HEIGHT+BORDER_SEPARATION));
			animateRect(null, Window.getClientHeight()-(GHAUiHelper.DEFAULT_NOTIFICATION_BUTTONS_HEIGHT+BORDER_SEPARATION), null, null);
		}else{
			...
			getBody().setHeight(GHAUiHelper.DEFAULT_NOTIFICATION_NOBUTTONS_HEIGHT-(HEADER_HEIGHT+BORDER_SEPARATION));
			animateRect(null, Window.getClientHeight()-(GHAUiHelper.DEFAULT_NOTIFICATION_NOBUTTONS_HEIGHT+BORDER_SEPARATION), null, null);
		}		
	}
}
