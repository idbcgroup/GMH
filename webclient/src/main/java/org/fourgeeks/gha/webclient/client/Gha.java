...
public class Gha implements EntryPoint {
	...
	public void onModuleLoad() {
		 ...
		 * 
		 * @Override public void onSuccess(Boolean result) { Window.alert("" +
		 * result); service.test2(new AsyncCallback<LegalEntity>() {
		 * 
		 ...
		 * }
		 * 
		 * @Override public void onSuccess(LegalEntity result) { if (result !=
		 * null) Window.alert("success"); else Window.alert("error");
		 * 
		 * } }); }
		 * 
		 * 
		...
		final SubmitItem submitButton = new SubmitItem("submitButton",
				"Iniciar Sesion");
		final CheckboxItem rememberData = new CheckboxItem("rememberData",
				"Recordar mis Datos");
		final LinkItem forgotData = new LinkItem("¿Olvido su Contraseña?");

		loginForm.setFields(username, password, submitButton, rememberData,
				forgotData);
