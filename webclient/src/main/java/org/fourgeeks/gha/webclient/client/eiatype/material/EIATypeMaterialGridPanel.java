...
public class EIATypeMaterialGridPanel extends VLayout implements
...

	{
		...
		materialSearchForm.AddMaterialSelectionListener(new MaterialSelectionListener() {
			
			@Override
			public void select(Material material) {
				final EiaTypeMaterial eiaTypeMaterial = new EiaTypeMaterial();
				eiaTypeMaterial.setEiaType(EIATypeMaterialGridPanel.this.eiaType);
				eiaTypeMaterial.setMaterial(material);
				EIATypeMaterialModel.save(eiaTypeMaterial, new GHAAsyncCallback<EiaTypeMaterial>() {
	...
	public EIATypeMaterialGridPanel() {
		...
		VLayout sideButtons = GHAUiHelper.createBar(
				...
				new GHAImgButton("../resources/icons/delete.png", new ClickHandler() {
					...
					@Override
					public void onClick(ClickEvent event) {
						EiaTypeMaterial eiaTypeMaterial = ((EIATypeMaterialRecord) eiaTypeMaterialGrid.getSelectedRecord()).toEntity();
						EIATypeMaterialModel.delete(eiaTypeMaterial.getId(), new GHAAsyncCallback<Void>() {
	...
	private void loadData() {
		EIATypeMaterialModel.find(eiaType, new GHAAsyncCallback<List<EiaTypeMaterial>>() {

			@Override
			public void onSuccess(List<EiaTypeMaterial> eiaTypeMaterial) {
				ListGridRecord[] array = EIATypeMaterialUtil.toGridRecords(eiaTypeMaterial)
						.toArray(new EIATypeMaterialRecord[] {});
				eiaTypeMaterialGrid.setData(array);
			}
		});
	}
