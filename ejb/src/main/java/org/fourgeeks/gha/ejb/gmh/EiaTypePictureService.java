...
public class EiaTypePictureService implements EiaTypePictureServiceRemote {
	...
	public List<EiaTypePicture> find(EiaType eiaType) throws EJBException {
		...
		// String query =
		// "SELECT e from EiaTypePicture e WHERE eiaTypeFk=:eiaTypeId order by id";
		...
		// }catch(Exception ex){
		// logger.log(Level.INFO, "Error buscando EiaTypePicture por EiaType ",
		// ex);
	...
	public boolean update(EiaTypePicture eiaTypePicture) throws EJBException {
		try {
			...
			throw new EJBException("Error actualizando eiaTypePicture "
					+ e.getCause().getMessage());
		}
	}
